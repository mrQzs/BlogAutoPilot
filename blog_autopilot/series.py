...
def detect_series(
    ...
    """
    try:
        return _detect_series_impl(db, tags, embedding, title)
...
def build_series_navigation(series_info: SeriesInfo) -> str:
    ...
    if series_info.prev_article and series_info.prev_article.url:
        prev_title = series_info.prev_article.title
        prev_url = series_info.prev_article.url
        prev_link = (
    ...
    return (
        ...
        f'    \U0001f4da 本文属于系列：《{series_info.series_title}》'
...
def build_backfill_navigation(
    ...
    if prev_article and prev_article.url:
        prev_link = (
            f'    <a href="{prev_article.url}" style="color:#1a73e8;text-decoration:none;">'
            f'\u2190 上一篇：{prev_article.title}</a>\n'
        )

    next_link = (
        f'    <a href="{next_article_url}" style="color:#1a73e8;text-decoration:none;">'
        f'下一篇：{next_article_title} \u2192</a>\n'
    )
