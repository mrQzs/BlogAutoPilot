...
- **封面图生成** — AI 根据文章标题生成抽象风格封面图并上传到 WordPress，支持备用 API 自动切换
...
AI_COVER_IMAGE_FALLBACK_API_KEY=    # 可选，备用封面图 API
AI_COVER_IMAGE_FALLBACK_API_BASE=   # 可选，备用封面图 API 地址
AI_MODEL_COVER_IMAGE_FALLBACK=      # 可选，备用模型（空则沿用主模型）
...
| 封面图生成 | 3 次 + 备用 API | 主 API 重试 3 次后自动切换备用 API |
